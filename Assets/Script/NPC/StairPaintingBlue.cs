...
public class StairPaintingBlue : MonoBehaviour
{
    ...
    private void Start()
    {
        player = GameObject.Find("Enemy Blue");
    ...
    {
        if (other.gameObject.tag == "Stairs" || other.gameObject.tag == "Yellow Stairs" || other.gameObject.tag == "Red Stairs" || other.gameObject.tag == "White Stairs")
        {
            if (brickTransformParent.transform.childCount > 0)
            {
                other.gameObject.GetComponent<MeshRenderer>().enabled = true;
                other.gameObject.GetComponent<MeshRenderer>().material = stairsMat;
                other.gameObject.tag = "Blue Stairs";
